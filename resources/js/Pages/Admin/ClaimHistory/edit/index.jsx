...
export default function Edit({ onSuccess, claimHistories }) {
    const { employes, prizes } = usePage().props;

    const { isOpen, onOpen, onOpenChange } = useDisclosure();
    const [selectedEmployee, setSelectedEmployee] = useState(
        employes.find((emp) => emp.id === claimHistories.employe_id),
    );

    // console.log(claimHistories.prize_id, "selectedEmployee");
    const [selectedPrize, setSelectedPrize] = useState(claimHistories.prize);
    const [availablePoints, setAvailablePoints] = useState(0);

    const { data, setData, put, processing, errors, reset } = useForm({
        employe_id: claimHistories.employe_id || "",
        prize_id: claimHistories.prize_id || "",
        QTY: claimHistories.QTY || 1,
    });


    useEffect(() => {
        // Fetch employee's available points on component mount
        if (claimHistories.employe_id) {
            fetchEmployeePoints(claimHistories.employee_id);
        }
    }, []);
    const handleSubmit = (e) => {
        console.log(data);
        e.preventDefault();
        put(route("admin.claim-history.update", claimHistories.id), {
            onSuccess: () => {
                reset();
                onOpenChange(false);
                if (onSuccess) {
    ...
    const fetchEmployeePoints = async (employeeId) => {
        try {
            ...
            // Add back the points from this claim since they're currently deducted
            const currentClaimPoints =
                claimHistories.prize.Point * claimHistories.QTY;
            setAvailablePoints(result.available_points + currentClaimPoints);
        } catch (error) {
