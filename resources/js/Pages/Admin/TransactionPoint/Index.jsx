...
export default function Index(props) {
    const { transactionpoints } = props;
        const columns = [
            { name: "Nama Karyawan", uid: "employe_id" },
            { name: "Entitle Date", uid: "EntitleDate" },
            { name: "Point Desc", uid: "PointDesc" },
        ...
        // Filter transactionpoints based on search keyword
        const filteredTransactionPoints = useMemo(() => {
            if (!keyword) return transactionpoints;

            return transactionpoints.filter((prize) => {
                const searchTerm = keyword.toLowerCase();
                return (
                    prize.EntitleDate?.toLowerCase().includes(searchTerm) ||
                    prize.PointType?.toLowerCase().includes(searchTerm) ||
                    prize.PointValue?.toLowerCase().includes(searchTerm) ||
                    prize.PointDesc?.toLowerCase().includes(searchTerm)
                );
            });
        }, [transactionpoints, keyword]);

        const pages = Math.ceil(filteredTransactionPoints.length / limit);

        const items = useMemo(() => {
            const start = (page - 1) * limit;
            const end = start + limit;

            return filteredTransactionPoints.slice(start, end);
        }, [page, filteredTransactionPoints, limit]);
        ...
        const handlePageChange = useCallback(
            (newPage) => {
                const maxPages = Math.ceil(filteredTransactionPoints.length / limit);
                if (newPage <= maxPages && newPage >= 1) {
                    setPage(newPage);
                }
            },
            [filteredTransactionPoints.length, limit],
        );

        const handleDataRefresh = useCallback(() => {
            router.reload({
                only: ["transactionpoints"],
                preserveScroll: true,
                preserveState: false,
            });
        }, []);
        ...
        const handleConfirmDelete = useCallback(() => {
            ...
            setIsDeleting(true);
            router.delete(route("admin.transaction-point.destroy", tPToDelete.id), {
                onSuccess: () => {
                    setIsDeleting(false);
                    setTPToDelete(null);
                    onOpenChange(false);
        ...
        const bottomContent = useMemo(() => {
            return (
                <div className="py-2 px-2 flex justify-between items-center">
                    <span className="w-[30%] text-small text-default-400">
                        {filteredTransactionPoints.length === 0
                            ? keyword
                                ? "No transaction points found matching your search"
                                : "No transaction points"
                            : `${items.length} of ${filteredTransactionPoints.length} transaction points${keyword ? " (filtered)" : ""}`}
                    </span>
                    <Pagination
                        showShadow
                        color="primary"
            ...
            pages,
            items.length,
            filteredTransactionPoints.length,
            handlePageChange,
            keyword,
        ]);

        const renderCell = useCallback(
            (transactionpoint, columnKey) => {
                const cellValue = transactionpoint[columnKey];
                switch (columnKey) {
                    case "employe_id":
                        return transactionpoint.NameEmployee;
                    case "PointType":
                        return transactionpoint.PointType == 'plus' ? '+' : '-';
                    case "actions":
                        return (
                                ...
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        <Button
                                            ...
                                            onPress={() => handleDeleteClick(transactionpoint)}
                                        >
                                            Delete
                                        </Button>
                                    </div>
